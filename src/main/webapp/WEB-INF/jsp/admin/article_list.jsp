        ...
        <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
            ...
            <li class="nav-item">
                <div class="btn-group">
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item" href="javascript:void(0);" onclick="fullScreen('审核注册申请','/admin/user/state')">审核注册申请</a>
                        <a class="dropdown-item" href="javascript:void(0);" onclick="fullScreen('查看用户详情','/admin/user/list')">查看用户详情</a>
                    </div>
                </div>
            </li>
            <li class="nav-item ">
...
<nav aria-label="Page navigation example" style="position: absolute;bottom: 10px;left: 42%">
    <ul class="pagination justify-content-center">
        <li class="page-item  <c:if test="${pageInfo.pageNum==1}">disabled</c:if> ">
            <a class="page-link" <c:if test="${empty word}">href="/admin/article/list?page=1"</c:if><c:if test="${!empty word}">href="/admin/article/search?page=1&word=${word}" </c:if> >&laquo;</a>
        </li>
        <c:forEach begin="1" end="${pageInfo.pages}" step="1" var="pageNo">
            <li class="page-item <c:if test="${pageInfo.pageNum==pageNo}">active</c:if>"><a class="page-link" <c:if test="${empty word}">href="/admin/article/list?page=${pageNo}"</c:if><c:if test="${!empty word}">href="/admin/article/search?page=${pageNo}&word=${word}"</c:if>>${pageNo}</a></li>
        </c:forEach>
        <li class="page-item  <c:if test="${pageInfo.pageNum==pageInfo.pages}">disabled</c:if> ">
            <a class="page-link" <c:if test="${empty word}">href="/admin/article/list?page=${pageInfo.pages}"</c:if><c:if test="${! empty word}">href="/admin/article/search?page=${pageInfo.pages}&word=${word}"</c:if>>&raquo;</a>
        </li>
    </ul>
</nav>
